...
<Steps>
  ...
  <Step title="Add the domain in updates.page">
    Go to **Settings → Portal → Domain Name**, click **Connect domain name**, and enter your full domain (e.g., `changelog.yourcompany.com`).
  </Step>
  ...
  <Step title="Check DNS">
    Click **Check DNS**. Once the status reads **Verified**, SSL is provisioned automatically.
  </Step>
</Steps>
...
<AccordionGroup>
  <Accordion title="Domain won't verify">
    ...
    - Wait a few minutes for DNS propagation, then click **Check DNS** again
