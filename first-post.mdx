---
title: Your First Post
description: 'Write and publish your first changelog entry'
---

# Creating Your First Post

Let's walk through creating a great first post for your changelog.

## What makes a good post?

Before we dive in, here are some tips:

<CardGroup cols={2}>
  <Card title="Be specific" icon="bullseye">
    "Improved dashboard performance by 40%" beats "Made things faster"
  </Card>
  <Card title="Lead with value" icon="star">
    Start with what users can now do, not what you built
  </Card>
  <Card title="Keep it scannable" icon="list">
    Use headings, bullets, and short paragraphs
  </Card>
  <Card title="Show, don't tell" icon="image">
    Screenshots and GIFs help users understand changes
  </Card>
</CardGroup>

## Step-by-step guide

### 1. Start a new post

From the Posts page, click the **+** button in the top right.

### 2. Write a compelling title

Your title should be clear and specific. Users should know what the update is about at a glance.

<Tabs>
  <Tab title="Good titles ✓">
    - "New: Dark mode is here"
    - "Faster exports with bulk download"
    - "Fixed: Login issues on Safari"
    - "API v2 now available"
  </Tab>
  <Tab title="Weak titles ✗">
    - "New feature"
    - "Bug fixes"
    - "Update"
    - "v2.3.1"
  </Tab>
</Tabs>

### 3. Write your content

Use the rich text editor to describe your update. A good structure:

```markdown
## What's new
Brief description of the feature or change.

## How it works
Step-by-step or key details.

## Why we built this
Optional: context on user feedback or your roadmap.
```

### 4. Choose a category

Categories help users filter and scan your changelog. We create three by default:

| Category | When to use |
|----------|-------------|
| **New** | Brand new features |
| **Improvement** | Enhancements to existing features |
| **Fix** | Bug fixes and corrections |

<Tip>
  You can create custom categories in **Settings → Changelog → Categories**
</Tip>

### 5. Add a cover image (optional)

Cover images make your posts more eye-catching, especially when shared on social media.

**Recommended specs:**
- Size: 1200 x 630px (2:1 ratio)
- Format: PNG or JPG
- Keep text minimal — it may be cropped on some platforms

### 6. Publish

Click **Publish**, then **Now**. Your post is live — open it from the post menu with **Open in Portal** to see it on your changelog.

<Note>
  **Pro tip:** **Save draft** keeps your work private, and the publish dialog's **Later** option [schedules](/posts/scheduling) the post for a future date.
</Note>

---

## Post templates

Here are some templates to get you started:

<AccordionGroup>
  <Accordion title="New Feature">
    ```markdown
    We're excited to introduce [Feature Name]!

    ## What's new
    [Brief description of what users can now do]

    ## How to use it
    1. Go to [location]
    2. Click [button]
    3. [Next step]

    ## What's next
    We're already working on [related improvement]. Stay tuned!
    ```
  </Accordion>
  <Accordion title="Improvement">
    ```markdown
    We've made [Feature] even better.

    ## What changed
    - [Improvement 1]
    - [Improvement 2]
    - [Improvement 3]

    ## Why we made this change
    Based on your feedback, we [reason].

    Let us know what you think!
    ```
  </Accordion>
  <Accordion title="Bug Fix">
    ```markdown
    We've fixed an issue affecting [description].

    ## What was happening
    [Brief description of the bug]

    ## What we fixed
    [What users can expect now]

    Thanks to everyone who reported this issue!
    ```
  </Accordion>
</AccordionGroup>

---

## Next steps

<Card title="Writing great posts" icon="pen" href="/posts/writing">
  Deep dive into post best practices
</Card>
